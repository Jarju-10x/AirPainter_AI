...
ret, frm = cap.read()
mask = np.ones((frm.shape[0], frm.shape[1]), dtype=np.uint8) * 255
    ...
    if op.multi_hand_landmarks:
        for i in op.multi_hand_landmarks:
            ...
            for id, lm in enumerate(i.landmark):
                lx, ly = int(lm.x * frm.shape[1]), int(lm.y * frm.shape[0])
                cv2.circle(frm, (lx, ly), 5, (0, 255, 0), cv2.FILLED)
                cv2.putText(frm, str(id), (lx, ly), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
